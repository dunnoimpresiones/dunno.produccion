...
function renderBambuFarmV2(){
  ...
  const cards=bambuPrintersV2.length?bambuPrintersV2.map(printer=>{
    ...
    return `<div class="bambu-card">
      ...
      <div class="bambu-bottom-grid"><div class="bambu-production-controls"><label>Pedido<select class="bambu-order-select" onchange="assignBambuOrderV2('${js(printer.id)}',this.value)"><option value="">Seleccionar pedido</option>${orderOptions}</select></label>${assignedOrder?`<div class="bambu-quantity-controls"><span class="bambu-quantity-value">${Number(assignedOrder.done)||0}/${Number(assignedOrder.qty)||0}</span><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',-1)">−</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',1)">+</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',10)">+10</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',20)">+20</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',25)">+25</button><button type="button" class="bambu-complete-button" onclick="setBambuDoneV2('${js(printer.id)}','ALL')">COMPLETADO</button></div>`:""}</div>
