...
function renderBambuFarmV2(){
  ...
  const cards=bambuPrintersV2.length?bambuPrintersV2.map(printer=>{
    ...
    return `<div class="bambu-card">
      ...
      <div class="bambu-order-bar"><select class="bambu-order-select" onchange="assignBambuOrderV2('${js(printer.id)}',this.value)"><option value="">Seleccionar pedido</option>${orderOptions}</select></div>
      ...
      <div class="bambu-bottom-grid"><div class="bambu-production-controls">${assignedOrder?`<div class="bambu-quantity-controls"><span class="bambu-quantity-value">${Number(assignedOrder.done)||0}/${Number(assignedOrder.qty)||0}</span><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',-1)">−</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',1)">+</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',10)">+10</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',20)">+20</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',25)">+25</button><button type="button" class="bambu-complete-button" onclick="setBambuDoneV2('${js(printer.id)}','ALL')">COMPLETADO</button></div>`:""}</div><div class="bambu-filaments"><strong>FILAMENTOS</strong><div class="bambu-color-summary">${filamentColumns.map((items,column)=>`<div class="bambu-filament-column"><b>Slot ${column+1}:</b>${items.length?items.map(item=>`<span title="${esc(item.color.name)}"><i style="background:${item.color.hex}"></i>${esc(item.color.name)}</span>`).join(""):"<small>-</small>"}</div>`).join("")}</div><button type="button" class="small-btn bambu-config-button" onclick="openBambuFilamentModalV2('${js(printer.id)}')">⚙ Configurar filamentos</button></div></div>
