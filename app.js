...
function cleanMachineOrders() {
  ...
  if(changed){
    saveMachines();
    machines.forEach(m=>{
      if(!m.orderId) queueMachineChangeV2(m);
    });
  }
}
...
function loadPendingMachinesV2(){
  try{
    const saved=JSON.parse(localStorage.getItem(PENDING_MACHINES_KEY_V2)||"{}");
    return saved&&typeof saved==="object"?saved:{};
  }catch(error){console.error("No se pudieron cargar cambios de máquinas:",error);return {}}
}
function savePendingMachinesV2(){
...
function clearDraftMachineColorsV2(){
  draftMachineColorsV2=[];
  renderColorPaletteV2();
}
...
function renderBambuFarmV2(){
  ...
  const cards=bambuPrintersV2.length?bambuPrintersV2.map(printer=>{
    ...
    return `<div class="bambu-card">
      <div class="bambu-card-head"><div class="bambu-printer-title">${esc(printer.name||"Bambu")}</div><span class="bambu-state ${state.toLowerCase()}">${state}</span></div>
      <div class="bambu-order-bar"><select class="bambu-order-select" onchange="assignBambuOrderV2('${js(printer.id)}',this.value)"><option value="">Seleccionar pedido</option>${orderOptions}</select>${assignedOrder?`<strong class="bambu-selected-order">${esc(assignedOrder.design||assignedOrder.product||"Sin nombre")} <small>(#${esc(assignedOrder.id)})</small></strong>`:""}</div>
      ...
      <div class="bambu-bottom-grid"><div class="bambu-filaments"><strong>FILAMENTOS</strong><div class="bambu-color-summary">${filamentColumns.map((items,column)=>`<div class="bambu-filament-column"><b>Slot ${column+1}:</b>${items.length?items.map(item=>`<span title="${esc(item.color.name)}"><i style="background:${item.color.hex}"></i>${esc(item.color.name)}</span>`).join(""):"<small>-</small>"}</div>`).join("")}</div><button type="button" class="small-btn bambu-config-button" onclick="openBambuFilamentModalV2('${js(printer.id)}')">⚙ Configurar filamentos</button></div><div class="bambu-production-controls">${assignedOrder?`<div class="bambu-quantity-controls"><span class="bambu-quantity-value">${Number(assignedOrder.done)||0}/${Number(assignedOrder.qty)||0}</span><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',-1)">−</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',1)">+</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',10)">+10</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',20)">+20</button><button type="button" onclick="setBambuDoneV2('${js(printer.id)}',25)">+25</button><button type="button" class="bambu-complete-button" onclick="setBambuDoneV2('${js(printer.id)}','ALL')">COMPLETADO</button></div>`:""}</div></div>
...
window.addEventListener("online",()=>flushPendingChangesV2());
